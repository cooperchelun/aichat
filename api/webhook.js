...
module.exports = async (req, res) => {
  if (req.method === 'POST') {
    try {
      ...
      // =================【功能一：輸入「原神」看版本】=================
      if (userMessage === '原神') {
        replyMsg = `【原神目前版本情報】\n\n🎮 官方當前最新版本：v6.6\n✨ 伺服器狀態：正常運作中\n\n💡 提示：現在不需要任何指令，直接打角色名（如：胡桃、林尼）就能自動爬取最新數據！`;
      } 
      // =================【功能二：查詢兌換碼】=================
      ...
      // =================【功能三：真．無條件強制角色數據挖掘】=================
        ...
        // 1. 先用 Fandom API 透過中文「搜尋」該角色的官方英文頁面名稱
        ...
        if (response.ok) {
          ...
          // 正規表示法動態精準挖掘側邊欄欄位數據（Element / Weapon / Birthday）
          ...
          if (detectedArtifacts.length > 0) {
            artifactReply = detectedArtifacts.slice(0, 2).map(art => {
              ...
              return art.replace(' (Artifact Set)', '').replace(' Set', '');
            }).join(' / ');
          }
